...
> **Versión actual:** `1.2.1` · Los planes de auditoría/revisión ya ejecutados se conservan (no versionados) en `.docs/archive/`.
...
| --------------------------------------------- | ---------------------------------------- | -------------------------------------------------------------------------------- |
| Instalación / sudoers / reglas dinámicas      | [Guía de Instalación](INSTALLATION.md)   | [§ Advanced Networking Setup](INSTALLATION.md#5-advanced-networking-setup)       |
...
curl http://localhost:8000/api/video/stats                  # Métricas de vídeo
curl http://localhost:8000/api/vpn/status                   # Estado VPN
curl http://localhost:8000/api/network/link-profile         # Perfil de enlace activo
...
curl http://localhost:8000/api/mavlink-router/outputs       # Salidas del router MAVLink
```
