...
echo "🔐 FPV Copilot Sky - Setting up sudoers for user: $FPVCOPILOT_USER"
...
for OLD_FILE in /etc/sudoers.d/tailscale /etc/sudoers.d/fpvcopilot-tailscale /etc/sudoers.d/fpvcopilot-wifi /etc/sudoers.d/fpvcopilot-system; do
    if [ -f "$OLD_FILE" ]; then
        echo "  🗑️  Removing legacy sudoers file: $OLD_FILE"
        rm -f "$OLD_FILE"
    fi
done
...
# Used for: up/down, mtu, txqueuelen — wildcard needed for dynamic interface names
...
# --- Sysctl — RESTRICTED to specific network parameters (network_optimizer.py) ---
...
# --- Ethtool — RESTRICTED to disabling Wake-on-LAN (network_optimizer.py:298) ---
...
if visudo -c -f "$SUDOERS_FILE" > /dev/null 2>&1; then
    ...
    echo "    ❌ sysctl -w *    → ✅ sysctl -w net.ipv4.tcp_congestion_control=*, etc."
    echo "    ❌ mkdir -p *     → ✅ mkdir -p /etc/dnsmasq.d"
    echo "    ❌ ethtool -s *   → ✅ ethtool -s * wol d"
    echo "    ❌ nmcli modify * → removed (unused by code)"
    echo "    ❌ nmcli dev set  → removed (unused by code)"
else
